...
let errlog_title = typeof actionflow_name !== "undefined" ? actionflow_name : "actionflow_frame"; //errlog表的title,错误日志记录
let debug = false; // 调试模式下不会记录日志，正式上线时请将调试模式关闭
let is_multi_app = false; //是否开启多应用,默认关闭
let default_system_system = null; //默认的多应用id,system数据表id,根据system_id传入后进行重置
